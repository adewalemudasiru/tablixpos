...
export function checkTrialExpired(trialStartedAt?: string): boolean {
  ...
  // Trial period is now 1 year (365 days).
  // To revert to the original 7-day trial, change the comparison back to:
  // return daysDiff > 7;
  return daysDiff > 365
}
...
interface AppContextValue extends AppState {
  ...
  consumeIngredients: (
    soldItems: { menuItemId: string; qty: number }[],
    performedBy?: string
  ) => void
    ...
    if (raw) {
      ...
      const migrateInvItem = (raw: Record<string, unknown>): InventoryItem => {
        ...
        const parseQtyStr = (s: string) => {
          ...
          return m
            ? { num: parseFloat(m[1]) || 0, unit: m[2].trim() }
            : { num: 0, unit: "" }
        }
      ...
      const migrateMenuItem = (raw: Record<string, unknown>): MenuItem => ({
        ...
        ingredients: Array.isArray(raw.ingredients)
          ? (raw.ingredients as MenuIngredient[])
          : [],
      })
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [plan, setPlanState] = useState<AppPlan>(() => {
    try {
      return (localStorage.getItem("tablix_plan") as AppPlan) || "trial"
    } catch (_) {
      return "trial"
    }
  })
  const [kotEnabled, setKotEnabled] = useState<boolean>(() => {
    try {
      return localStorage.getItem("tablix_kot_enabled") === "true"
    } catch (_) {
      return false
    }
  })
  const [tablesEnabled, setTablesEnabled] = useState<boolean>(() => {
    try {
      return localStorage.getItem("tablix_tables_enabled") === "true"
    } catch (_) {
      return false
    }
  })
  const [restaurantName, setRestaurantName] = useState<string>(() => {
    try {
      return localStorage.getItem("tablix_restaurant_name") || "My Restaurant"
    } catch (_) {
  ...
  const setTaxConfig = useCallback(
    (cfg: TaxConfig) => setTaxConfigState(cfg),
    []
  )

  const [posConfig, setPosConfigState] = useState<PosConfig>(
    () => initial.posConfig
  )
  const setPosConfig = useCallback(
    (cfg: PosConfig) => setPosConfigState(cfg),
    []
  )
  ...
  const setBusinessConfig = useCallback(
    (cfg: BusinessConfig) => setBusinessConfigState(cfg),
    []
  )
  ...
  const setLoyaltyConfig = useCallback(
    (cfg: LoyaltyConfig) => setLoyaltyConfigState(cfg),
    []
  )

  const [logo, setLogoState] = useState<string | null>(() => {
    try {
      return localStorage.getItem("tablix_logo") || initial.logo
    } catch (_) {
      return initial.logo
    }
  })
  ...
  const [theme, setThemeState] = useState<"light" | "dark">(() => {
    try {
      return (
        (localStorage.getItem("tablix_theme") as "light" | "dark") || "dark"
      )
    } catch (_) {
      return "dark"
    }
  ...
  const setTheme = useCallback((t: "light" | "dark") => {
    setThemeState(t)
    try {
      localStorage.setItem("tablix_theme", t)
    } catch (_) {}
  }, [])
  ...
  useEffect(() => {
    try {
      localStorage.setItem("tablix_kot_enabled", String(kotEnabled))
    } catch (_) {}
  }, [kotEnabled])
  useEffect(() => {
    try {
      localStorage.setItem("tablix_tables_enabled", String(tablesEnabled))
    } catch (_) {}
  }, [tablesEnabled])
  useEffect(() => {
    try {
      localStorage.setItem("tablix_restaurant_name", restaurantName)
    } catch (_) {}
  }, [restaurantName])
  useEffect(() => {
    try {
      localStorage.setItem("tablix_menu_url", menuUrl)
    } catch (_) {}
  }, [menuUrl])
  useEffect(() => {
    try {
      localStorage.setItem("tablix_plan", plan)
    } catch (_) {}
  }, [plan])
  ...
  const addMenuItem = useCallback(
    (item: MenuItem) => setMenuItems((p) => [...p, item]),
    []
  )
  ...
  const deleteMenuItem = useCallback(
    (id: string) => setMenuItems((p) => p.filter((i) => i.id !== id)),
    []
  )
  const addMenuCategory = useCallback(
    (cat: MenuCategory) => setMenuCategories((p) => [...p, cat]),
    []
  )
  const updateMenuCategory = useCallback(
    (cat: MenuCategory) =>
      setMenuCategories((p) => p.map((c) => (c.id === cat.id ? cat : c))),
    []
  )
  const deleteMenuCategory = useCallback(
    (id: string) => setMenuCategories((p) => p.filter((c) => c.id !== id)),
    []
  )

  // ── Inventory ─────────────────────────────────────────────────────────────
  const addInventoryItem = useCallback(
    (item: InventoryItem) => setInventoryItems((p) => [...p, item]),
    []
  )
  ...
  const deleteInventoryItem = useCallback(
    (id: string) => setInventoryItems((p) => p.filter((i) => i.id !== id)),
    []
  )
  const addInventoryLogEntry = useCallback(
    (entry: InventoryLogEntry) => setInventoryLog((p) => [entry, ...p]),
    []
  )
  ...
  const stockIn = useCallback(
    (ingredientId: string, qty: number, performedBy: string, note?: string) => {
      const today = new Date().toISOString().split("T")[0]
      const time = new Date().toISOString().split("T")[1].split(".")[0]
  ...
  const addSupplier = useCallback(
    (s: Supplier) => setSuppliers((p) => [...p, s]),
    []
  )
  const updateSupplier = useCallback(
    (s: Supplier) => setSuppliers((p) => p.map((x) => (x.id === s.id ? s : x))),
    []
  )
  const deleteSupplier = useCallback(
    (id: string) => setSuppliers((p) => p.filter((s) => s.id !== id)),
    []
  )

  // ── Customers ─────────────────────────────────────────────────────────────
  const addCustomer = useCallback(
    (c: StoreCustomer) => setCustomers((p) => [...p, c]),
    []
  )
  ...
  const deleteCustomer = useCallback(
    (id: string) => setCustomers((p) => p.filter((c) => c.id !== id)),
    []
  )
  const addLoyaltyPoints = useCallback(
    (customerId: string, points: number, spent: number) => {
      const today = new Date().toISOString().split("T")[0]
      setCustomers((p) =>
        p.map((c) =>
          c.id === customerId
  ...
  const addStaffMember = useCallback(
    (s: StoreStaff) => setStaff((p) => [...p, s]),
    []
  )
  const updateStaffMember = useCallback(
    (s: StoreStaff) => setStaff((p) => p.map((x) => (x.id === s.id ? s : x))),
    []
  )
  const deleteStaffMember = useCallback(
    (id: string) => setStaff((p) => p.filter((s) => s.id !== id)),
    []
  )

  const addRole = useCallback((r: StoreRole) => setRoles((p) => [...p, r]), [])
  const updateRole = useCallback(
    (r: StoreRole) => setRoles((p) => p.map((x) => (x.id === r.id ? r : x))),
    []
  )
  const deleteRole = useCallback(
    (id: string) => setRoles((p) => p.filter((x) => x.id !== id)),
    []
  )

  const addStation = useCallback(
    (s: string) => setStations((p) => [...p, s]),
    []
  )
  const deleteStation = useCallback(
    (s: string) => setStations((p) => p.filter((x) => x !== s)),
    []
  )

  // ── Expenses ──────────────────────────────────────────────────────────────
  const addExpense = useCallback(
    (e: Expense) => setExpenses((p) => [e, ...p]),
    []
  )
  ...
  const deleteExpense = useCallback(
    (id: string) => setExpenses((p) => p.filter((e) => e.id !== id)),
    []
  )
  ...
  const voidTransaction = useCallback(
    (id: string, approvedBy: string, refund = false) => {
  ...
  // ── KDS ───────────────────────────────────────────────────────────────────
  const addKDSOrder = useCallback(
    (order: KDSOrder) => setKdsOrders((p) => [order, ...p]),
    []
  )
  const updateKDSOrder = useCallback(
    (order: KDSOrder) =>
      setKdsOrders((p) => p.map((o) => (o.id === order.id ? order : o))),
    []
  )
  const removeKDSOrder = useCallback(
    (id: string) => setKdsOrders((p) => p.filter((o) => o.id !== id)),
    []
  )
  ...
  const addTable = useCallback(
    (t: RestaurantTable) => setTables((p) => [...p, t]),
    []
  )
  const updateTable = useCallback(
    (t: RestaurantTable) =>
      setTables((p) => p.map((x) => (x.id === t.id ? t : x))),
    []
  )
  const deleteTable = useCallback(
    (id: string) => setTables((p) => p.filter((t) => t.id !== id)),
    []
  )
  const setTableStatus = useCallback(
    (id: string, status: TableStatus, extra?: Partial<RestaurantTable>) => {
      setTables((p) =>
        p.map((t) => (t.id === id ? { ...t, status, ...extra } : t))
      )
    },
    []
  )
  ...
  const isReadOnly =
    plan === "trial" && checkTrialExpired(businessConfig.trialStartedAt)
...
export function getActiveKDSOrderCount(orders: KDSOrder[]) {
  return orders.filter(
    (o) =>
      o.status === "New" || o.status === "In Progress" || o.status === "Ready"
  ).length
}
...
export function nextOrderNo(kdsOrders: KDSOrder[]): string {
  const nums = kdsOrders
    .map((o) => parseInt(o.orderNo.replace("ORD-", ""), 10))
    .filter((n) => !isNaN(n))
