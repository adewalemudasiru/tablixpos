  ...
  try {
    ...
    if (data.accessToken) {
      ...
      if (data.refreshToken)
        localStorage.setItem("tablixpos_refresh_token", data.refreshToken)
      return true
    }
  } catch (_) {}
  return false
...
export const authAPI = {
  ...
  login: async (email: string, pin: string): Promise<AuthResponse> => {
    const user = findMockUser(email, pin)

    if (!user) {
      const error = new Error("Invalid email or PIN") as ApiError
      error.status = 401
      error.data = { message: "Invalid email or PIN. Please try again." }
      throw error
    }
...
export const staffAPI = {
  ...
  create: (data: {
    ...
    pin: string
    assignedStation?: string
  }): Promise<{ success: boolean; data: { staff: ApiStaff } }> =>
  ...
  loginWithPin: (
    ...
    if (!businessId) {
      return Promise.reject(
        new Error("Business context not found. Please log in as owner first.")
      )
    }
...
export interface ApiMenuIngredient {
  ingredientId: string
  qty: number
}
...
export const menuAPI = {
  listCategories: (): Promise<{
    success: boolean
    data: { categories: ApiMenuCategory[] }
  }> => apiCall("/menu/categories"),

  createCategory: (data: {
    name: string
    description?: string
  }): Promise<{ success: boolean; data: { category: ApiMenuCategory } }> =>
  ...
  createItem: (data: {
    ...
    ingredients?: ApiMenuIngredient[]
    station?: string
  }): Promise<{ success: boolean; data: { item: ApiMenuItem } }> =>
    apiCall("/menu/items", { method: "POST", body: JSON.stringify(data) }),

  updateItem: (
    id: string,
    data: Partial<Omit<ApiMenuItem, "id" | "createdAt">>
  ): Promise<{ success: boolean; data: { item: ApiMenuItem } }> =>
...
export const tableAPI = {
  ...
  create: (data: {
    ...
    seats?: number
    zone?: string
  }): Promise<{ success: boolean; data: { table: ApiTable } }> =>
...
export const businessAPI = {
  getProfile: (): Promise<{
    success: boolean
    data: { business: ApiBusinessProfile }
  }> => apiCall("/business/profile"),

  updateProfile: (
    data: Partial<Omit<ApiBusinessProfile, "id" | "updatedAt">>
  ): Promise<{ success: boolean; data: { business: ApiBusinessProfile } }> =>
...
export const inventoryAPI = {
  list: (params?: {
    ...
    success: boolean
    data: { ingredients: ApiIngredient[]; pagination: Pagination }
  ...
  create: (data: {
    ...
    unitCost?: number
    supplier?: string
  }): Promise<{ success: boolean; data: { ingredient: ApiIngredient } }> =>
  ...
  getLogs: (
    id: string
  ): Promise<{ success: boolean; data: { logs: ApiInventoryLog[] } }> =>
...
export const customersAPI = {
  list: (params?: {
    ...
    success: boolean
    data: { customers: ApiCustomer[]; pagination: Pagination }
  ...
  create: (data: {
    ...
    phone: string
    email?: string
  }): Promise<{ success: boolean; data: { customer: ApiCustomer } }> =>
...
export const expensesAPI = {
  list: (params?: {
    ...
    success: boolean
    data: { expenses: ApiExpense[]; pagination: Pagination }
  ...
  create: (data: {
    ...
    category: ExpenseCategory
    expenseDate: string
  }): Promise<{ success: boolean; data: { expense: ApiExpense } }> =>
...
export type OrderStatus =
  "Pending" | "InProgress" | "Ready" | "Served" | "Cancelled"
...
export const ordersAPI = {
  ...
  ): Promise<{ success: boolean; data: { order: ApiOrder } }> =>
  ...
  list: (params?: {
    ...
    success: boolean
    data: { orders: ApiOrder[]; pagination: Pagination }
...
export const transactionsAPI = {
  list: (params?: {
    ...
    success: boolean
    data: { transactions: ApiOrder[]; pagination: Pagination }
...
export const subscriptionAPI = {
  ...
  verify: (data: {
    ...
    success: boolean
    data: { plan: SubscriptionPlan; expiresAt: string }
  }> =>
    apiCall("/subscriptions/verify", {
      method: "POST",
      body: JSON.stringify(data),
    }),
...
export const settingsAPI = {
  get: (): Promise<{
    success: boolean
    data: { settings: ApiBusinessSettings }
  }> => apiCall("/settings"),

  update: (
    data: Partial<ApiBusinessSettings>
  ): Promise<{ success: boolean; data: { settings: ApiBusinessSettings } }> =>
...
export const suppliersAPI = {
  ...
  create: (
    data: Omit<ApiSupplier, "id" | "createdAt">
  ): Promise<{ success: boolean; data: { supplier: ApiSupplier } }> =>
    apiCall("/suppliers", { method: "POST", body: JSON.stringify(data) }),

  update: (
    id: string,
    data: Partial<Omit<ApiSupplier, "id" | "createdAt">>
  ): Promise<{ success: boolean; data: { supplier: ApiSupplier } }> =>
