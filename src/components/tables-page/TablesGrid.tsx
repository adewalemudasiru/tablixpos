...
export function TablesGrid({
  ...
  return (
    <div className="flex items-start justify-center gap-12 py-8 lg:flex-row lg:gap-24">
      {/* Left: Square/Standard tables */}
      {squareTables.length > 0 && (
        <div className="flex w-[300px] shrink-0 flex-col gap-4">
          <div className="grid gap-4 grid-cols-[repeat(auto-fit,minmax(min(140px,100%),1fr))]">
      ...
      {roundTables.length > 0 && (
        <div className="flex w-[140px] shrink-0 flex-col items-center gap-4">
          <div className="grid w-full gap-4 grid-cols-[repeat(auto-fit,minmax(min(140px,100%),1fr))]">
            <AnimatePresence mode="popLayout">
              {roundTables.map((t) => (
                <div key={t.id} className="h-[140px] w-[140px]">
                  <TableCard
